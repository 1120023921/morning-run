        ...
        try {
            ...
            readResult.forEach(row -> {
                ...
                try {
                    ...
                    log.error(JSONArray.toJSONString(row));
                    throw new RuntimeException("转换excel失败");
                }
            });
            if (!saveBatch(tsUserList)) {
                log.error(JSONObject.toJSONString(tsUserList));
                throw new RuntimeException("导入学生信息失败");
            }
            if (!tsUserRoleService.saveBatch(tsUserRoleList)) {
                log.error(JSONObject.toJSONString(tsUserRoleList));
